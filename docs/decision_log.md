...
### Decision: Stealth Mechanic in `hard` tasks
- **Alternatives**: Only use standard localized failures; don't include tricky context.
- **Chosen**: Implement a stealth incident where a root cause degrades subtly while downstream dependents emit critical, loud alerts.
- **Rationale**: To test true operational reasoning rather than naive heuristic matching. If standard benchmarks only test straight mapping, agents don't learn causal backtracking. By introducing stealth metrics (e.g. "gradual memory leak") acting as the true root cause, we reliably bait naive agents into misidentifying downstream systems.

### Decision: Dynamic Cascade spawning scope and grading
- **Alternatives**: Spawn cascade alerts natively as new ground truth, heavily penalizing agents globally.
- **Chosen**: Spawns 1 deterministic downstream alert per un-triaged critical/high baseline alert after step 5. New alerts impact per-step reward but are explicitly excluded from the final Grader.
- **Rationale**: Adding dynamically spawned alerts creates a temporal urgency requiring triage prioritization. However, factoring them into the final grader dramatically harms reproducibility and cross-agent comparability. Keeping them out of the grader ensures a stable baseline while imposing step-to-step operational pressure.

### Decision: Minimal `inference.py` fallback and system prompt redesign
- **Alternatives**: Include the full "answer key" (metrics to root causes) directly into `inference.py` to ensure high performance on baseline.
- **Chosen**: Strip exhaustive metric->cause mapping. Replace explicit string-matching lookup logic with a single "skip" payload fallback that safely yields an action without embedding actual task intelligence.
- **Rationale**: Improves credibility. A robust benchmark shouldn't provide the answer sheet inside the baseline inference script. Stripping the heuristic fallback ensures the baseline correctly fails (scores near 0) when the LLM service drops, behaving like an honest test suite.
